...
if ( ! function_exists( 'mwp_setup' ) ) :
	...
	function mwp_setup() {
		 ...
		 * If you're building a theme based on m, use a find and replace
		 * to change 'mwp' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'mwp', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'main-nav' => esc_html__( 'Primary', 'mwp' ),
			)
		);
 ...
 */
function mwp_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'mwp_content_width', 640 );
}
add_action( 'after_setup_theme', 'mwp_content_width', 0 );
...
function mwp_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'mwp' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'mwp' ),
 ...
 */
function mwp_scripts() {
	wp_enqueue_style( 'mwp-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'mwp-style', 'rtl', 'replace' );

	wp_enqueue_script( 'mwp-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'mwp_scripts' );
...
function add_theme_scripts() {
	...
	wp_enqueue_script( 'fontawesome', '//kit.fontawesome.com/db563f905a.js', array (), 1.1, true);
	wp_enqueue_script( 'jquery351', '//ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js', array(), '3.5.1', true );
	wp_enqueue_script( 'slick-js', '//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js', array('jquery'), '3.5.1', true );
	wp_enqueue_script( 'bootstrap', '//stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js', array ( 'jquery' ), 1.1, true);
	wp_enqueue_script( 'bs-dropdown-override', get_template_directory_uri() . '/assets/js/bs-dropdown-override.js', array ( 'jquery' ), 1.1, true);
	wp_enqueue_script( 'search-toggle', get_template_directory_uri() . '/assets/js/search-toggle.js', array ( 'jquery' ), 1.1, true);
  }
